...
std::string getFileNameFromIdentifier(hid_t obj_id) {
  ssize_t sz = H5Fget_name(obj_id, nullptr, 0);
  if (sz < 0) throw Exception("Cannot get file name", ioda_Here());
  std::vector<char> data(sz + 1, 0);
  ssize_t ret = H5Fget_name(obj_id, data.data(), data.size());
  if (ret < 0) throw Exception("Cannot get file name", ioda_Here());
